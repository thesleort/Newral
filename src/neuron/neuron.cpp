    ...
    if (fmc->layer_type != INPUT) {
        ...
        for (int offset_x = 0; offset_x < fmc->size + 2 * fmc->padding; (offset_x = offset_x + fmc->stride)) {
            ...
            for (int offset_y = 0; offset_y < fmc->size + 2 * fmc->padding; (offset_y = offset_y + fmc->stride)) {
                ...
                fmc->layer_prev->filter[filter].neuron[offset_x][offset_y].set_output_weight(this, x, y);
                m_input_weights[x][y] = fmc->layer_prev->filter[filter].neuron[offset_x][offset_y].get_output_weight(x, y)->edge;
            }
        }
    }
}
