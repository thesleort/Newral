  ...
  while (getline(m_cfg_file, line)) {
    ...
    if (layer_type == INPUT) {
      ...
      if (fields.at(0).compare("filters") == 0) {
        ...
        std::cout << "layer conv filters: "
                  << current_layer->filters_config->filters << "\n";
...
void setup::allocator() {
  ...
  for (unsigned layer_num = 0; layer_num < m_net_config.layers.size(); ++layer_num) {
    ...
    Layer &previous_layer = (layer_num > 0)
                                ? m_net_config.layers.at(layer_num - 1)
                                : m_net_config.layers.at(layer_num);
    ...
    case CONVOLUTION:
      ...
      width = (previous_layer.width -
               current_layer.filters_config->width +
               2 *
                   current_layer.filters_config->padding) /
                  current_layer.filters_config->stride +
              1;

      height = (previous_layer.height -
                current_layer.filters_config->height +
                2 *
                    current_layer.filters_config->padding) /
                   current_layer.filters_config->stride +
               1;
    ...
    case MAXPOOL:
      ...
      // Calculate size of layer
      width = (previous_layer.width - current_layer.maxpool_config->width) /
                  current_layer.maxpool_config->stride +
              1;
      height = (previous_layer.height - current_layer.maxpool_config->height) /
                   current_layer.maxpool_config->stride +
               1;
      ...
      for (int i = 0; i < layers.getLength(); i++) {
        ...
        if (id.compare(m_net_config.layers[layer_num].id) == 0) {
          ...
          const unsigned filter_size =
              m_net_config.layers[layer_num].filters_config->width *
              m_net_config.layers[layer_num].filters_config->height *
              m_net_config.layers[layer_num].filters_config->depth;
          for (filter_num = 0;
               filter_num < m_net_config.layers[layer_num].num_filters;
               ++filter_num) {
            for (weight_num = 0;
                 weight_num < filter_size;
                 weight_num++) {
              m_net_config
                  .layers[layer_num]
                  .filters[filter_num]
                  .filter_weights[weight_num] =
                  (float)filters[filter_num][weight_num];
            }
            m_net_config
                .layers[layer_num]
                .filters[filter_num]
                .bias = (float)layers[i]["bias"][filter_num];
            std::cout << "\n";
          }
          break;
        }
    ...
    case FULLY: {
      for (int i = 0; i < layers.getLength(); i++) {
        ...
        if (id.compare(m_net_config.layers[layer_num].id) == 0) {
          ...
          const unsigned filter_size = m_net_config
                                           .layers[layer_num]
                                           .fully_config->size;
          ...
          for (filter_num = 0;
            ...
            for (weight_num = 0; weight_num < filter_size; weight_num++) {
              m_net_config
                  .layers[layer_num]
                  .weights[filter_num]
                  .net_weights[weight_num] =
                  (float)weights[filter_num][weight_num];
            }
            m_net_config
                .layers[layer_num]
                .weights[filter_num]
                .bias = (float)layers[i]["bias"][0];
          }
        }
        break;
      }
...
float *setup::load_input(std::string &input_file, bool is_image) {
  int input_size = m_net_config.input_width *
